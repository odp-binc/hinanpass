x18n.register 'ja',
  ...
  description:
    title: '「ヒナンパス」とは'
    contents: '
      <p>ヒナンパスとは、いざというときに役に立つ、
      もしものときのスマートフォン向け災害別避難所案内Webサービスです。</p>
      <p>自治体が公開したオープンデータの避難所情報とスマートフォンの現在地情報を利用して、
      近隣の避難所を地図画面上に表示します。表示された避難所の場所をタッチするだけで、
      避難所の情報や避難所へのルート案内を行ってくれます。</p>
      <p>また当サービスでは「地震」「洪水」…といった、災害の種類別に、ケースバイケースの避難所案内・
      避難方法案内に対応しています。そのため突然の災害で対策が手薄になっている場合も、
      当サービスを利用すれば災害ごとに適切な避難が行えます。</p>
      <p>現在はまだ少数地域の情報しかありませんが、
      オープンデータプラットフォーム(odp※)に避難所情報オープンデータが追加登録される度に
      随時対応地域が拡大します</p>
      <p>当アプリの情報はodpにて提供されている観光情報オープンデータを利用しています。
      よって当アプリにて表示する情報はodpに参加されている自治体地域の情報となります。</p>
  ...
  caution:
    title: '利用にあたって'
    contents: '
      <p>当サービスでは以下の様ないくつかの制限がございますが、
      ご理解の上ご利用いただけたらと思います。</p>
      <h4>位置情報について</h4>
      <p>位置情報の取得時に、何らかの要因でGPS情報が正確に取得されないとき、
      地図上に現在地情報が正しく表示されない場合がございます。
      あらかじめご了承の上、なるべく屋外の見通しのよい場所にてご利用ください。</p>
      <h4>現在地情報利用制限時の現在地検索について</h4>
      <p>当サービスでは、スマートフォンの現在地情報の利用を許可していない方のために
      現在地を検索出来る機能を提供しております。こちらの検索された位置情報ですが、
      同名の地名や入力ミスなどにより、現在地が特定できなかったり、誤った現在地を表示するおそれがあります。
      正確な位置情報を使って利用されたい場合は、現在地情報のスマートフォンや
      Webブラウザなどでの利用を許可していただくようお願いします。</p>
      <h4>表示される避難所について</h4>
      <p>当サービスでは処理速度向上のため、オープンデータで公開されている避難所情報のうち、
      所在地が現在地の半径10[km]圏内のもののみを取得・表示しています。
      徒歩における避難では問題になることは少ないかと思いますが、
      現在地が大きく変化した場合はWebブラウザから再読み込みを行ってください。</p>
      <h4>情報について</h4>
      <p>対象スポットの詳細データは、自治体様がodpにデータを登録したものを参照しておりますので、
      最新の情報が出ない場合もございます。</p>'
